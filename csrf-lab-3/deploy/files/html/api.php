...
if ($method == 'POST' && isset($_POST['action']) && $_POST['action'] == 'delete' && isset($_POST['id'])) {
    ...
    if (isset($_SESSION['admin_token']) && (time() < $_SESSION['admin_token_expire']) && $currentUser === 'admin' && $_POST['csrf_token'] === $_SESSION['admin_token']) {
        $currentUser = 'admin';
    } 
    elseif (!isset($_SESSION['csrf_token']) || $_SESSION['csrf_token'] !== $_POST['csrf_token']) {
        http_response_code(403); // CSRF token不匹配，返回403
        echo json_encode(['error' => '无效的CSRF token']);
        exit; // 退出
    }
